...
import { thunkSetUser } from '../../../store/User/actions';
...
export const SignupForm = () => {
  ...
  return (
    <form className='flex flex-col max-w-[28rem] w-full mb-4' onSubmit={handleSubmit(onSubmit)}>
      ...
      <label htmlFor='signup-email' className='flex flex-col w-full mb-1 text-[14px] font-bold'>
        Ваш адрес электронной почты.
      </label>
      <input className='px-4 py-3 w-full border my-2'
             id='signup-email'
             type='text'
      ...
      <label htmlFor='signup-email-confirm' className='flex flex-col w-full mb-1 text-[14px] font-bold'>
      ...
      <label htmlFor='signup-password' className='flex flex-col w-full mb-1 text-[14px] font-bold'>
        Придумайте пароль
      </label>
      <input className='px-4 py-3 w-full border my-2'
             id='signup=password'
             ...
             {...register('password', {
               ...
               validate: {
                 onlyLatin: value => /[a-zA-Z]/g.test(value) || 'Password should contain only latins letter and digits',
                 lowerLetter: value => /(?=.*[a-z])/.test(value) || 'Password should contain at least one lower case letter',
                 upperLetter: value => /(?=.*[A-Z])/.test(value) || 'Password should contain at least one upper case letter',
                 digit: value => /(?=.*\d)/.test(value) || 'Password should contain at least one digit',
               },
             })}
      />
      {errors.password && <ErrorMessage message={errors.password.message!}/>}

      <label htmlFor='signup-name' className='flex flex-col w-full mb-1 text-[14px] font-bold'>
        Ваше имя
      </label>
      <input className='px-4 py-3 w-full border my-2'
             id='signup-name'
      ...
      <label className='flex flex-col w-full mb-1 text-[14px] font-bold' htmlFor='signup-birthday'>
        Ваша дата рождения
      </label>
      <div className='flex'>
        <input className='px-4 py-3 w-full border my-2 mr-4'
               id='signup-birthday'
               type='number'
               min={1}
               max={31}
               ...
               placeholder='ДД'/>
        <select className='px-4 py-3 w-full border my-2 mr-4'
                defaultValue={''}
                {...register('month', {
                  required: 'Please select your birth month',
                })}
        >
          <option disabled={true} value=''>Месяц</option>
        ...
        <input className='px-4 py-3 w-full border my-2 appearance-none'
               type='number'
               min={1900}
               max={2022}
               {...register('year', {
      ...
      <div className='flex flex-col mb-8'>
        <div className='flex items-center'>
          ...
          />
          <label className='text-[14px] mr-6' htmlFor='man'>Мужчина</label>
          ...
          />
          <label className='text-[14px]' htmlFor='woman'>Женщина</label>
        </div>
      ...
      <div className='flex items-start mb-4'>
        ...
        <label htmlFor='signup-ads'>Я не хочу получать рекламные сообщения от Spotify.</label>
      </div>

      <div className='flex items-start mb-4'>
        <input className='mt-1 mr-2' type='checkbox' id='signup-info'/>
        <label htmlFor='signup-info'>Я разрешаю сообщить мои регистрационные данные партнерам Spotify в целях
          рекламы.</label>
      </div>

      <div className='flex flex-col mb-8'>
        <div className='flex items-start'>
          <input className='mt-1 mr-2'
                 id='signup-policy'
                 type='checkbox'
                 {...register('policy', {
          ...
          />
          <label htmlFor='signup-policy'>Я принимаю Условия использования и Политику конфиденциальности
            Spotify.</label>
        </div>
      ...
      >
        <If condition={loading}>
          <svg role='status' className='inline mr-3 w-6 h-6 text-white animate-spin' viewBox='0 0 100 101'
